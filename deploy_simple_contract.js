...
async function main() {
  try {
    ...
    console.log("SIMPLIFIED FLASH LOAN ARBITRAGE CONTRACT DEPLOYMENT");
    console.log("=".repeat(80));
    ...
    // Use the standalone simplified contract
    console.log("\nCompiling simplified contract...");
    const simplifiedContractPath = './build_temp/FlashLoanArbitrage_Simple.sol';
    
    // Compile the contract with solc (with overwrite flag)
    try {
      const compileCommand = `solc --bin --abi --optimize --optimize-runs 200 --overwrite -o ./build_temp ${simplifiedContractPath}`;
    ...
    const contractBin = fs.readFileSync('./build_temp/FlashLoanArbitrage.bin', 'utf8');
    const contractAbi = JSON.parse(fs.readFileSync('./build_temp/FlashLoanArbitrage.abi', 'utf8'));
    ...
    const gasLimit = 3000000; // Lower limit for simpler contract
    ...
    // Initialize the contract with minimal parameters
    console.log("\nInitializing contract with minimal parameters...");
    ...
    console.log(`PancakeV3Factory: ${contractInfo._pancakeV3Factory}`);
    console.log(`PancakeV2Router: ${contractInfo._pancakeV2Router}`);
    console.log(`Is Setup: ${contractInfo._isSetup}`);
    ...
    console.log("=".repeat(80));
    console.log(`Contract Address: ${contract.address}`);
    console.log(`Owner Address: ${contractInfo._owner}`);
