    ...
    PausableUpgradeable 
{
    ...
    function withdraw(address token, uint256 amount) external onlyOwner nonReentrant {
        ...
        if (token == address(0)) {
            ...
            require(amount <= balance, "Insufficient BNB balance");
    ...
    /**
     * @dev Update slippage cap for a DEX
     * @param dex Name of the DEX ("pancakeV3", "pancakeV2", "apeSwap", "biSwap")
     * @param slippageCapBps New slippage cap in basis points
     */
    function updateSlippageCap(
        string calldata dex,
        uint256 slippageCapBps
    ) external onlyOwner {
        require(slippageCapBps <= 1000, "Slippage cap too high"); // Max 10%
        
        bytes32 dexHash = keccak256(abi.encodePacked(dex));
        
        if (dexHash == keccak256(abi.encodePacked("pancakeV3"))) {
            pancakeV3SlippageCap = slippageCapBps;
        } else if (dexHash == keccak256(abi.encodePacked("pancakeV2"))) {
            pancakeV2SlippageCap = slippageCapBps;
        } else if (dexHash == keccak256(abi.encodePacked("apeSwap"))) {
            apeSwapSlippageCap = slippageCapBps;
        } else if (dexHash == keccak256(abi.encodePacked("biSwap"))) {
            biSwapSlippageCap = slippageCapBps;
        } else {
            revert("Invalid DEX name");
        }
        
        emit SlippageCapUpdated(dex, slippageCapBps);
    }
    
    /**
     * @dev Update MEV protection settings
     * @param enabled Whether MEV protection is enabled
     * @param enableBackrun Whether to enable backrun protection
     * @param enableFrontrun Whether to enable frontrun protection
     * @param feeMultiplier Priority fee multiplier (100 = 1x, 150 = 1.5x)
     */
    function updateMevProtection(
        ...
        require(feeMultiplier >= 100 && feeMultiplier <= 500, "Invalid fee multiplier"); // 1x to 5x
        
        mevProtectionEnabled = enabled;
        backrunEnabled = enableBackrun;
        frontrunEnabled = enableFrontrun;
        priorityFeeMultiplier = feeMultiplier;
        
        emit MevProtectionUpdated(enabled, enableBackrun, enableFrontrun);
    }
     ...
     * @return _totalTrades Total number of arbitrage attempts
     * @return _successfulTrades Number of successful trades
     * @return _totalProfit Total profit made
     */
    ...
    /**
     * @dev Handle incoming BNB (required for receiving BNB)
     */
